RSpec.shared_examples_for 'a KrbEncKey' do |expected:|
  describe '#data' do
    it 'has a data hash' do
      expect(subject.data.keys - %i[ enctype key salt ]).to be_empty
      expect(subject.data[:enctype]).to eq(expected[:enctype])
      expect(subject.data[:key].bytes.length).to eq(expected[:key_length])
      expect(subject.data[:salt]).to match(expected[:salt])
    end
  end
...
RSpec.describe Metasploit::Credential::KrbEncKey, type: :model do
  ...
  context 'when the krb enc key is not a known enctype' do
    subject :krb_enc_key_aes256 do
      FactoryBot.build(:metasploit_credential_krb_enc_key, data: { enctype: 1024, key: "abc" })
    end
